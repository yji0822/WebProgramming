...
-- 22/04/22 금요일 SQL 활용 평가 (1)
-- 1. 생성된 테이블의 목록을 출력하는 SQL문을 작성하시오(5점).

SELECT * FROM TAB;

-- 2. TO_DATE 단일행함수를 이용하여 EMP 테이블에서 HIREDATE가 
...
--    사원명, 급여, 부서번호, 부서명, 근무지, 직속상사명을 상사가 없는 직원까지 
--    전체 직원 다 출력하는 SQL문을 작성하시오. 
...
--    EMP 테이블의 사원을 사번, 사원명, 직책, 급여, 급여등급을 
--    급여 기준 내림차순으로 정렬하여 출력하는 SQL문을 작성하시오(5점).
...
-- 6. EMP 테이블에서 직책별 급여 평균이 3000이상인 직책에 대하여 
--    직책명, 평균급여, 급여의 합을 출력하는 SQL문을 작성하시오(5점).
...
-- 7. EMP테이블에서 사원명이 ‘FORD’인 직원과 
--    직책(JOB)과 급여(SAL)이 같은 사원의 모든 정보를 출력하는 SQL문을 작성하시오(5점).
...
-- 9. 데이터베이스에서 트랜잭션은 데이터 처리의 한 단위를 말한다. 
--    데이터를 조작하기 위한 INSERT, UPDATE, DELETE 등의 명령들이 실행됨과 동시에 트랜잭션이 진행되는데, 
--    이들 INSERT, UPDATE, DELETE 작업이 성공적으로 데이터베이스에 반영·처리되도록 명령어를 작성하시오(5점).

COMMIT;

-- 10. EMP테이블에서 사원명으로 조회하는 시간을 단축하기 위해 사용되는 인덱스 생성 명령어를 작성하시오. 
--     생성된 인덱스의 이름은 IDX_EMP_ENAME으로 한다(10점).

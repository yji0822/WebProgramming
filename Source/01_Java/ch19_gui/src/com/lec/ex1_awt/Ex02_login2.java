...
public class Ex02_login2 extends Frame {
	...
	public Ex02_login2(String title) { // 반드시 실행해야하는 생성자
	// Frame에 컴포넌트 추가하고 화면에 보이게 > 이벤트
		super(title); // 반드시 맨 첫번째 줄에
		// Frame의 레이아웃 타입 : BorderLayout (동서남북, 센터) 기본값. - 따로 지정을 안하면 출력하기 전에 지정(따로 생성 안해도됨)
		//                   FlowLayout(왼쪽부터 오른쪽으로, 위부터 아래로 차곡차곡 컴포넌트가 add 되는 것)
		//                   GridLayout(몇행 몇열)
		panel = new Panel(); // 패널의 기본 레이아웃 : FlowLayout
//		setLayout(new GridLayout(2, 2)); 잘 사용 안한다.

...
-- 조회수 조작 (3번글이 제일 핫한글)
UPDATE BOARD SET READCOUNT = 70 WHERE NUM = 150;
...
UPDATE BOARD SET RE_STEP = RE_STEP+1 WHERE REF=150 AND RE_STEP>0;
INSERT INTO BOARD (NUM, WRITER, SUBJECT, CONTENT, EMAIL, PW, REF, RE_STEP, RE_INDENT, IP)
    VALUES (152, '답변자2', '글150-2', '본문', 'WOW@W.COM','1',
                    150, 1, 1, '192.168.1.12');

-- 글의 RE_STEP는 오름차순 정렬 REF는 내림차순 정렬
SELECT * FROM BOARD ORDER BY REF DESC, RE_STEP;
COMMIT;


-- 150번 원글의 답변글인 152번글의 답변글 만들기
SELECT * FROM BOARD WHERE NUM = 152; -- 152번 글이 원글이 된다.
    -- ⓐ STEP
...
-- NUM에 153이 아닌 (SELECT NVL(MAX(NUM),0) + 1 FROM BOARD) 이걸 넣어주어야 한다.
